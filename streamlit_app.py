...
# ==================================================================================================
# V0.18.10 · SEMÁNTICA PROFESIONAL V2-R1.2
# Ajuste quirúrgico posterior a LAB2-D.4
# ==================================================================================================

SEMANTICA_PROFESIONAL_VERSION_V01810 = "V2-R1.2"


# --------------------------------------------------------------------------------------------------
# COBERTURA ADICIONAL DE TÍTULOS
#
# Reconocer un título NO implica automáticamente recomendarlo.
# Las nuevas familias quedan deliberadamente conservadoras.
# --------------------------------------------------------------------------------------------------

FAMILIAS_CARGO_V01810.setdefault(
    "analisis_financiero",
    []
)
...
# Relaciones deliberadamente menores al umbral 60 de cargo objetivo.
RELACIONES_FAMILIAS_V01810.update({
...
#   datos    -> tecnologia
#
# Datos -> software solo podrá avanzar mediante evidencia técnica explícita.
# --------------------------------------------------------------------------------------------------
...
SOFTWARE_EVIDENCIA_FUERTE_V01812 = [
...
def familia_cargo_v01810(
    ...
    if "business analyst" in cargo_norm:

        return "business_analysis"


    if "product analyst" in cargo_norm:

        return "product_analysis"


    if "product manager" in cargo_norm:

        return "product_management"


    if "project manager" in cargo_norm:

        return "project_management"


    if "scrum master" in cargo_norm:
...
# --------------------------------------------------------------------------------------------------
# CLASIFICACIÓN DE EXPERIENCIA R1.2
# --------------------------------------------------------------------------------------------------
...
def clasificar_experiencia_v01881(
    ...
    coincidencias_funcionales = (
        _contar_coincidencias_semantica_v01810(
            texto_exp,
            funciones_destino
        )
    )
    ...
    # Datos conserva exigencia técnica.
    if dominio_destino == "datos":
    ...
    # Software exige evidencia de desarrollo explícita.
    elif dominio_destino == "tecnologia":
        ...
        if (
            ...
            return {
                ...
                "razon":
                    (
                        "La experiencia contiene evidencia técnica explícita "
                        "de desarrollo útil para el dominio de software."
                    ),
    ...
    # 3. RELACIONADA
    #
    # Un 40-69 hacia software ya NO basta por título.
    # ----------------------------------------------------------------------------------------------

    if similitud >= 40:

        if dominio_destino != "tecnologia":
        ...
        if tecnicas_software >= 2:
            ...
            return {
                ...
                "razon":
                    (
                        "Existe relación profesional con software respaldada "
                        "por evidencia técnica explícita."
                    ),
    ...
    if (
        ...
        return {
            ...
            "razon":
                (
                    "La experiencia combina capacidades de alto valor "
                    "con una transición profesional plausible hacia "
                    "el dominio destino."
                ),
...
# === END V0.18.10 SEMANTICA PROFESIONAL V2 R1.2 ===
...
def evaluar_oferta_base_v01813(
    oferta,
    candidato,
    perfil
):
...
# ==================================================================================================
# V0.18.10 · INTEGRACIÓN DE RECOMENDACIONES R1.3
# Clasificación explícita:
# OFERTA_QUE_BUSCAS / TE_PODRIA_INTERESAR / NO_RECOMENDAR
# ==================================================================================================

INTEGRACION_RECOMENDACIONES_VERSION_V01813 = "R1.3"


def _numero_seguro_v01813(valor, defecto=0.0):
    try:
        return float(valor)
    except Exception:
...
def _score_funciones_v01813(detalle):
...
def _brechas_criticas_recomendacion_v01813(
    evaluacion
):
    ...
    for habilidad, datos in matches.items():
        ...
        importancia = _numero_seguro_v01813(
            datos.get(
                "importancia"
            )
        )

        cumplimiento = _numero_seguro_v01813(
            datos.get(
                "cumplimiento"
            )
        )
...
def clasificar_recomendacion_v01813(
    oferta,
    candidato,
    perfil,
    evaluacion,
    ...
    valido, motivo_bloqueo = oferta_valida(
        oferta,
        candidato,
    )

    afinidad = _numero_seguro_v01813(
        evaluacion.get(
            "cargo"
        )
    )

    conocimientos = _numero_seguro_v01813(
        evaluacion.get(
            "cumplimiento"
        )
    )

    funciones = _score_funciones_v01813(
        evaluacion.get(
            "experiencia_contextual_detalle",
            {},
        )
    ...
    experiencia_contextual = _numero_seguro_v01813(
        evaluacion.get(
            "experiencia_contextual_score"
        )
    )
    ...
    brechas_criticas = (
        _brechas_criticas_recomendacion_v01813(
            evaluacion
        )
    )
    ...
    if not valido:
        ...
        return {
            ...
            "recomendacion_motivo":
                f"Bloqueo duro: {motivo_bloqueo}",

            "recomendacion_afinidad_cargo":
                round(
                    afinidad,
                    2,
                ),

            "recomendacion_conocimientos":
                round(
                    conocimientos,
                    2,
                ),

            "recomendacion_funciones":
                round(
                    funciones,
                    2,
                ),

            "recomendacion_experiencia_contextual":
                round(
                    experiencia_contextual,
                    2,
                ),

            "recomendacion_capacidad":
                0.0,

            "recomendacion_senales_fuertes":
                [],

            "recomendacion_brechas_criticas":
                brechas_criticas,

            "recomendacion_version":
                INTEGRACION_RECOMENDACIONES_VERSION_V01813,
        }
    ...
    if (
        ...
        motivo = (
            "Afinidad directa con el cargo objetivo "
            "y sin brecha crítica."
        )
        ...
        motivo = (
            "Afinidad nominal menor, pero existe "
            "compatibilidad profesional suficiente "
            "por conocimientos, funciones y experiencia."
        )
        ...
        motivo = (
            "No alcanza los criterios mínimos "
            "de cargo objetivo ni de oportunidad "
    ...
    return {
        ...
        "recomendacion_experiencia_equivalente":
            round(
                _numero_seguro_v01813(
                    evaluacion.get(
                        "experiencia_equivalente"
                    )
                ),
...
def evaluar_oferta(
    ...
    evaluacion = evaluar_oferta_base_v01813(
        oferta,
        candidato,
        perfil,
    )
    ...
    metadata = clasificar_recomendacion_v01813(
        oferta,
        candidato,
        perfil,
        evaluacion,
...
# === END INTEGRACIÓN R1.3 · EVALUAR_OFERTA ===
...
def construir_ranking_base_v01813(
    candidato,
    perfil
):
...
def _extraer_categoria_recomendacion_v01813(
    item
):
...
def filtrar_ranking_recomendable_v01813(
    ranking
):
    ...
    for item in ranking:
        ...
        categoria = (
            _extraer_categoria_recomendacion_v01813(
                item
            )
        )
...
def separar_ranking_recomendaciones_v01813(
    ranking
):
...
def construir_ranking(
    ...
    resultado = construir_ranking_base_v01813(
        candidato,
        perfil,
    )

    # Contrato histórico real del portal:
    # construir_ranking() devuelve (ranking, descartadas).
    if (
        isinstance(
            resultado,
            tuple,
        ...
        ranking, descartadas = resultado

        ranking_filtrado = (
            filtrar_ranking_recomendable_v01813(
                ranking
            )
        )
    ...
    # Compatibilidad defensiva con harness/laboratorios
    # que devuelvan únicamente una lista.
    return filtrar_ranking_recomendable_v01813(
        resultado
    )

INTEGRACION_RECOMENDACIONES_CONTRATO_VERSION_V018131 = "R1.3.1"

# === END INTEGRACIÓN R1.3 · CONSTRUIR_RANKING ===
...
def render_usuario(
    ...
    with tab_rec:
        ...
        grupos_recomendaciones_v01814 = (
            separar_ranking_recomendaciones_v01813(
                ranking
            )
        )


        ofertas_objetivo_v01814 = (
            grupos_recomendaciones_v01814.get(
                "OFERTA_QUE_BUSCAS",
                []
            )
        )
        ...
        posiciones_ranking_v01814 = {
            id(elemento):
                posicion

            for posicion, elemento
            in enumerate(
                ranking,
                start=1
            )
        ...
        objetivo_indexado_v01814 = [
            (
                posiciones_ranking_v01814.get(
                    id(elemento),
                    posicion_fallback
                ),
                elemento
            ...
            for posicion_fallback, elemento
            in enumerate(
                ofertas_objetivo_v01814,
                start=1
            )
        ...
        tab_objetivo_v01814, tab_adyacente_v01814 = st.tabs(
            [
                (
                    "🎯 Ofertas que buscas "
                    f"({len(ofertas_objetivo_v01814)})"
                ),
                (
                    "✨ Te podría interesar "
                    f"({len(ofertas_adyacentes_v01814)})"
                )
            ]
        )
        ...
        with tab_objetivo_v01814:

            st.caption(
                "Oportunidades alineadas directamente con "
                "el cargo objetivo que definiste."
            )


            if not ofertas_objetivo_v01814:

                st.info(
                    "Por ahora no encontramos ofertas con "
                    "alineación directa suficiente con tu "
                    "cargo objetivo."
                )


            for posicion, elemento in objetivo_indexado_v01814:
                nombre = elemento[
                    "nombre"
                ]
        ...
        with tab_adyacente_v01814:

            st.caption(
                "Oportunidades relacionadas donde tu experiencia, "
                "conocimientos y funciones muestran una compatibilidad "
                "profesional relevante."
            )


            if not ofertas_adyacentes_v01814:

                st.info(
                    "Por ahora no encontramos oportunidades "
                    "profesionales relacionadas con evidencia "
                    "suficiente."
                )


            for posicion, elemento in adyacente_indexado_v01814:
                nombre = elemento[
                    "nombre"
                ]
